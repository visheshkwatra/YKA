import '../booking_details_screen/widgets/carnamelist_item_widget.dart';
import '../booking_details_screen/widgets/interiortext_item_widget.dart';
...
class BookingDetailsScreen extends StatelessWidget {
  const BookingDetailsScreen({Key? key})
      : super(
          key: key,
        );
  ...
  static Widget builder(BuildContext context) {
    return BlocProvider<BookingDetailsBloc>(
      create: (context) => BookingDetailsBloc(BookingDetailsState(
        bookingDetailsModelObj: BookingDetailsModel(),
      ))
        ..add(BookingDetailsInitialEvent()),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(
            height: 93.v,
            child: BlocSelector<BookingDetailsBloc, BookingDetailsState,
                BookingDetailsModel?>(
              selector: (state) => state.bookingDetailsModelObj,
              builder: (context, bookingDetailsModelObj) {
                return ListView.separated(
                  ...
                  itemCount:
                      bookingDetailsModelObj?.interiortextItemList.length ?? 0,
                  itemBuilder: (context, index) {
                    InteriortextItemModel model =
                        bookingDetailsModelObj?.interiortextItemList[index] ??
                            InteriortextItemModel();
                    return InteriortextItemWidget(
                      model,
                    );
                  },
                );
      ...
      child: BlocSelector<BookingDetailsBloc, BookingDetailsState,
        ...
        builder: (context, bookingDetailsModelObj) {
          return ListView.separated(
            ...
            itemCount: bookingDetailsModelObj?.carnamelistItemList.length ?? 0,
            itemBuilder: (context, index) {
              CarnamelistItemModel model =
                  bookingDetailsModelObj?.carnamelistItemList[index] ??
                      CarnamelistItemModel();
              return CarnamelistItemWidget(
                model,
              );
            },
          );
