import '../my_favourties_details_screen/widgets/cardetailslist1_item_widget.dart';
import '../my_favourties_details_screen/widgets/interiortext2_item_widget.dart';
import 'bloc/my_favourties_details_bloc.dart';
import 'models/cardetailslist1_item_model.dart';
import 'models/interiortext2_item_model.dart';
import 'models/my_favourties_details_model.dart';
...
class MyFavourtiesDetailsScreen extends StatelessWidget {
  const MyFavourtiesDetailsScreen({Key? key})
      : super(
          key: key,
        );

  static Widget builder(BuildContext context) {
    return BlocProvider<MyFavourtiesDetailsBloc>(
      create: (context) => MyFavourtiesDetailsBloc(MyFavourtiesDetailsState(
        myFavourtiesDetailsModelObj: MyFavourtiesDetailsModel(),
      ))
        ..add(MyFavourtiesDetailsInitialEvent()),
      child: MyFavourtiesDetailsScreen(),
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(
            height: 93.v,
            child: BlocSelector<MyFavourtiesDetailsBloc,
                MyFavourtiesDetailsState, MyFavourtiesDetailsModel?>(
              selector: (state) => state.myFavourtiesDetailsModelObj,
              builder: (context, myFavourtiesDetailsModelObj) {
                return ListView.separated(
                  ...
                  itemCount: myFavourtiesDetailsModelObj
                          ?.interiortext2ItemList.length ??
                      0,
                  itemBuilder: (context, index) {
                    Interiortext2ItemModel model = myFavourtiesDetailsModelObj
                            ?.interiortext2ItemList[index] ??
                        Interiortext2ItemModel();
                    return Interiortext2ItemWidget(
                      model,
                    );
                  },
                );
  ...
  Widget _buildCarDetailsList(BuildContext context) {
    return SizedBox(
      height: 87.v,
      child: BlocSelector<MyFavourtiesDetailsBloc, MyFavourtiesDetailsState,
          MyFavourtiesDetailsModel?>(
        selector: (state) => state.myFavourtiesDetailsModelObj,
        builder: (context, myFavourtiesDetailsModelObj) {
          return ListView.separated(
            scrollDirection: Axis.horizontal,
            ...
            itemCount:
                myFavourtiesDetailsModelObj?.cardetailslist1ItemList.length ??
                    0,
            itemBuilder: (context, index) {
              Cardetailslist1ItemModel model =
                  myFavourtiesDetailsModelObj?.cardetailslist1ItemList[index] ??
                      Cardetailslist1ItemModel();
              return Cardetailslist1ItemWidget(
                model,
              );
            },
          );
