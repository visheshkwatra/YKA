import '../profile_fortyseven_bottomsheet/widgets/widget3_item_widget.dart';
import 'bloc/profile_fortyseven_bloc.dart';
import 'models/profile_fortyseven_model.dart';
...
class ProfileFortysevenBottomsheet extends StatelessWidget {
  const ProfileFortysevenBottomsheet({Key? key})
      : super(
          key: key,
        );

  static Widget builder(BuildContext context) {
    return BlocProvider<ProfileFortysevenBloc>(
      create: (context) => ProfileFortysevenBloc(ProfileFortysevenState(
        profileFortysevenModelObj: ProfileFortysevenModel(),
      ))
        ..add(ProfileFortysevenInitialEvent()),
      child: ProfileFortysevenBottomsheet(),
    );
  }
  ...
  Widget _buildSaveAsColumn(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        BlocSelector<ProfileFortysevenBloc, ProfileFortysevenState,
            ProfileFortysevenModel?>(
          selector: (state) => state.profileFortysevenModelObj,
          builder: (context, profileFortysevenModelObj) {
            return Wrap(
              runSpacing: 16.v,
              spacing: 16.h,
              children: List<Widget>.generate(
                profileFortysevenModelObj?.widget3ItemList.length ?? 0,
                (index) {
                  Widget3ItemModel model =
                      profileFortysevenModelObj?.widget3ItemList[index] ??
                          Widget3ItemModel();

                  return Widget3ItemWidget(
                    model,
                    onSelectedChipView: (value) {
                      context.read<ProfileFortysevenBloc>().add(
  ...
  Widget _buildHouseFlatBlockNoEditText(BuildContext context) {
    return BlocSelector<ProfileFortysevenBloc, ProfileFortysevenState,
